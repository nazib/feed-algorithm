...
def Personalized_ranks(ranked_feed, ranked_user_info, user_data, selected_uid):
    ...
    user_weights = pd.DataFrame(data=np.zeros(shape=(
        ranked_user_info.shape[0], selected_uid.shape[1])), columns=user_data.columns)
    ...
    # pd.to_numeric(ranked_feed["RankScore"])
    global_ranks = ranked_feed["RankScore"].values

    personal_rank = np.zeros(shape=(user_weights.shape[0], 1), dtype=float)

    for i in range(user_weights.shape[0]):
        personal_rank[i] = np.sum(
            user_feature[i, :]*user_weights[i, :])*global_ranks[i]
